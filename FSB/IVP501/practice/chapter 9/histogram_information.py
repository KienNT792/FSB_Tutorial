"""
Khái niệm: Histogram cung cấp thông tin gì về ảnh?
Ví dụ: Phân tích thông tin từ histogram của ảnh grayscale và RGB.
"""
...
# Phân tích thông tin
print("Thông tin từ histogram ảnh grayscale:")
print(f"Tổng số pixel: {np.sum(hist_gray)}")
print(f"Mức sáng phổ biến nhất: {np.argmax(hist_gray)}")

print("\nThông tin từ histogram ảnh RGB:")
for color, hist in hist_rgb.items():
    print(f"Kênh {color.upper()}: Tổng số pixel: {np.sum(hist)}, Mức sáng phổ biến nhất: {np.argmax(hist)}")
